...
const checkAdmin = async (req, res, next) => { 
  ...
  const role = await guild.members.cache.filter(member => member.roles.cache.find(role => role.id === '869945590115622946')).map(member => member.id)
...
app.post('/start-mainmode', checkAuth, checkAdmin, async function(req, res) {
  rdb.get("MAINTENANCE").then(value => {
    if (value === "true") {
